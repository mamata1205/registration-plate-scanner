...
import org.platescanner.platescanner;
import org.platescanner.model.Coordinate;
import org.platescanner.model.Result;
import org.platescanner.model.Results;
...
public class ALPR {
    ...
    private void executeRecognition(final Mat m, final String country, final ResultsCallback callback, final WeakReference<Context> context, int rotation) {
        ...
        // image is need to have proper orientation before delivering to platescanner for recognition
        applyOrientation(m, true, rotation);

        // path to platescanner config file in android environment
        final String androidDataDir = ctx.getApplicationInfo().dataDir;
        final String platescannerConfFile = androidDataDir + File.separatorChar + "runtime_data" + File.separatorChar + "platescanner.conf";

        // create file in internal memory for passing to platescanner
        ...
        // synchronous call for license plate recognition to platescanner
        String result = platescanner.Factory.create(ctx, androidDataDir).recognizeWithCountryRegionNConfig(country, "", file.getAbsolutePath(), platescannerConfFile, 10);
    ...
    /**
     * returns android native coordinates from platescanner object
     */
